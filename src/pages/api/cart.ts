  ...
  try {
    ...
    const itemsHtml = items?.map((item: any) => `
      <tr>
        ...
        <td style="padding: 12px; border-bottom: 1px solid #eee; vertical-align: top;">
          <div style="font-weight: 600; color: #1a1a1a; margin-bottom: 4px; font-size: 15px;">${item.name[lang]}</div>
          ${item.description?.[lang] ? `<div style="font-size: 12px; color: #777; line-height: 1.4;">${item.description[lang]}</div>` : ''}
        </td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; text-align: center; vertical-align: top; font-weight: 500;">${item.quantity}</td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; text-align: right; vertical-align: top; white-space: nowrap;">${formatCurrency(item.price[lang], currency)}</td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; text-align: right; vertical-align: top; font-weight: 600; color: #1a1a1a; white-space: nowrap;">
          ${formatCurrency(item.price[lang] * item.quantity, currency)}
        </td>
      </tr>
    `).join('') || '';
    ...
    const emailHtml = `
      <div style="font-family: sans-serif; max-width: 600px; margin: auto; color: #333; line-height: 1.6;">
        ...
        <p>${L.thanks} <strong>${companyName[lang] || 'Store'}</strong>. ${L.details}</p>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: customerEmail,
        cc: [companyRecipient],
        subject: `[${companyName[lang] || 'Order'}] ${L.subject} ${name}`,
        html: emailHtml,
      }),
    });
